...
get.so2.in.year.online <-function(year,plants)
{
  so2.column.index       <- grep(so2.columns[year-1994], colnames(plants))
  ...
  df      <- plants[plants$FIRST_YEAR_OF_OPERATION<=year & 
                    ...
                    c(so2.column.index,fuel.type.index)]
...
fuel.all.ej<-(lapply(year.list2,plants.ej, FUN=get.so2.in.year.online))
fuel.all.nonej<-(lapply(year.list2,plants.nonej, FUN=get.so2.in.year.online))
...
so2.fuel.ej.summary    <- doBy::summary_by(freq ~ year, FUN=sum, data=fuel.ej[complete.cases(fuel.ej$SO2),])
so2.fuel.nonej.summary <- doBy::summary_by(freq ~ year, FUN=sum, data=fuel.nonej[complete.cases(fuel.nonej$SO2),])
bar.width <- 0.7
coord.fixed.ratio  <- 5
annotate.text.size <- 4
plot1.na <- ggplot(data=fuel.ej.sorted[order(fuel.ej.sorted$Fuel),],    aes(x=year, y=SO2, fill=order)) +
  geom_bar(stat="summary", fun= "mean", width = bar.width) + 
  ylim(ylim) + 
  ggtitle(title.ej) +
  scale_fill_manual(values=manual.colors.na, labels=fuel.labels) + 
  labs(x="Year", y="SO2 thousand short tons (mean)")  +
  annotate(geom="text", x=so2.fuel.ej.summary$year, y=(0.3), label=so2.fuel.ej.summary$freq.sum, color="black", size= annotate.text.size) +
...
plot2.na <- ggplot(data=fuel.nonej.sorted[order(fuel.nonej.sorted$Fuel),], aes(x=year, y=SO2, fill=order)) + 
  geom_bar(stat="summary", fun= "mean", width = bar.width) + 
  ylim(ylim) +
  ggtitle(title.nonej) + 
  scale_fill_manual(values=manual.colors.na, labels=fuel.labels) + 
  labs(x="Year", y="SO2 thousand short tons (mean)") +
  annotate(geom="text", x=so2.fuel.nonej.summary$year, y=(0.3), label=so2.fuel.nonej.summary$freq.sum, color="black", size= annotate.text.size) +
  theme(legend.position="bottom", 
        axis.text.x  = element_text(color = "grey20", size = 16, angle = 45, hjust = .5, vjust = .5, face = "plain"),
        axis.text.y  = element_text(color = "grey20", size = 12, angle = 0,  hjust = 1,  vjust = 0,  face = "plain"),  
        axis.title.x = element_text(color = "grey20", size = 16, angle = 0,  hjust = .5, vjust = 0,  face = "plain"),
